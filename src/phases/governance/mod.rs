...
pub async fn make_governance_phase(
    ...
    let name = "governance";
    let (policy, context_channel) = do_connect_governance_context(name, settings, clearinghouse_magnet).await?;

    let governance = PolicyPhase::with_transform(name, policy, make_governance_transform(name)).await;
