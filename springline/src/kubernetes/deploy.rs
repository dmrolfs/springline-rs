...
impl DeployApi {
    ...
    pub async fn get_scale(&self, correlation: &CorrelationId) -> Result<Option<i32>, KubernetesError> {
        ...
        let scale = match self {
            Self::StatefulSet { name, api } => api.get_scale(name).instrument(span).await.map_err(error::convert_kube_error),
            Self::Deployment { name, api } => api.get_scale(name).instrument(span).await.map_err(error::convert_kube_error),
        };
    ...
    pub async fn patch_scale(&self, replicas: usize, correlation: &CorrelationId) -> Result<Option<i32>, KubernetesError> {
