...
impl JobHealthMetricsStrategyBuilder {
    pub fn strategy() -> impl Strategy<Value = JobHealthMetrics> { Self::new().finish() }
    ...
    pub fn job_nr_completed_checkpoints(self, job_nr_completed_checkpoints: impl Strategy<Value = u32> + 'static) -> Self {
        let mut new = self;
...
pub struct FlowMetricsStrategyBuilder {
    ...
    source_total_lag: Option<BoxedStrategy<Option<u32>>>,
...
impl FlowMetricsStrategyBuilder {
    pub fn strategy() -> impl Strategy<Value = FlowMetrics> { Self::new().finish() }
    ...
    pub fn source_back_pressured_time_millie_per_sec(self, source_back_pressured_time_millie_per_sec: impl Strategy<Value = f64> + 'static) -> Self {
        let mut new = self;
        new.source_back_pressured_time_millis_per_sec = Some(source_back_pressured_time_millie_per_sec.boxed());
        new
    }

    pub fn just_source_back_pressured_time_millie_per_sec(self, source_back_pressured_time_millie_per_sec: impl Into<f64>) -> Self {
        self.source_back_pressured_time_millie_per_sec(Just(source_back_pressured_time_millie_per_sec.into()))
    }

    pub fn forecasted_timestamp(self, forecasted_timestamp: impl Strategy<Value = Option<Timestamp>> + 'static) -> Self {
        let mut new = self;
    ...
    pub fn forecasted_records_in_per_sec(self, forecasted_records_in_per_sec: impl Strategy<Value = Option<f64>> + 'static) -> Self {
        let mut new = self;
    ...
    pub fn source_assigned_partitions(self, source_assigned_partitions: impl Strategy<Value = Option<u32>> + 'static) -> Self {
        let mut new = self;
    ...
    pub fn source_total_lag(self, source_total_lag: impl Strategy<Value = Option<u32>> + 'static) -> Self {
        let mut new = self;
        new.source_total_lag = Some(source_total_lag.boxed());
        new
    }

    pub fn just_source_total_lag(self, source_total_lag: impl Into<Option<u32>>) -> Self {
        self.source_total_lag(Just(source_total_lag.into()))
    }

    pub fn source_records_consumed_rate(self, source_records_consumed_rate: impl Strategy<Value = Option<f64>> + 'static) -> Self {
        let mut new = self;
    ...
    pub fn source_millis_behind_latest(self, source_millis_behind_latest: impl Strategy<Value = Option<u32>> + 'static) -> Self {
        let mut new = self;
    ...
    pub fn finish(self) -> impl Strategy<Value = FlowMetrics> {
        ...
        let source_back_pressured_time_millis_per_sec = self.source_back_pressured_time_millis_per_sec.unwrap_or(any::<f64>().boxed());
        let forecasted_timestamp = self.forecasted_timestamp.unwrap_or(prop::option::of(arb_timestamp_after(Timestamp::now(), arb_range_duration(1..=1200))).boxed());
        let forecasted_records_in_per_sec = self.forecasted_records_in_per_sec.unwrap_or(prop::option::of(any::<f64>()).boxed());
        let source_records_lag_max = self.source_records_lag_max.unwrap_or(prop::option::of(any::<u32>()).boxed());
        let source_assigned_partitions = self.source_assigned_partitions.unwrap_or(prop::option::of(any::<u32>()).boxed());
        let source_total_lag = self.source_total_lag.unwrap_or(prop::option::of(any::<u32>()).boxed());
        let source_records_consumed_rate = self.source_records_consumed_rate.unwrap_or(prop::option::of(any::<f64>()).boxed());
        let source_millis_behind_latest = self.source_millis_behind_latest.unwrap_or(prop::option::of(any::<u32>()).boxed());
...
impl ClusterMetricsStrategyBuilder {
    pub fn strategy() -> impl Strategy<Value = ClusterMetrics> { Self::new().finish() }
    ...
    pub fn task_network_input_queue_len(self, task_network_input_queue_len: impl Strategy<Value = f64> + 'static) -> Self {
        let mut new = self;
    ...
    pub fn task_network_output_queue_len(self, task_network_output_queue_len: impl Strategy<Value = f64> + 'static) -> Self {
        let mut new = self;
    ...
    pub fn task_network_output_pool_usage(self, task_network_output_pool_usage: impl Strategy<Value = f64> + 'static) -> Self {
        let mut new = self;
