...
pub fn register_metrics(registry: &Registry) -> Result<()> {
    ...
    registry.register(Box::new(flink::METRIC_CATALOG_FLOW_TASK_UTILIZATION.clone()))?;
    registry.register(Box::new(flink::METRIC_CATALOG_FLOW_SOURCE_RECORDS_LAG_MAX.clone()))?;
    registry.register(Box::new(flink::METRIC_CATALOG_FLOW_SOURCE_ASSIGNED_PARTITIONS.clone()))?;
    registry.register(Box::new(flink::METRIC_CATALOG_FLOW_SOURCE_TOTAL_LAG.clone()))?;
    registry.register(Box::new(
        flink::METRIC_CATALOG_FLOW_SOURCE_RECORDS_CONSUMED_RATE.clone(),
    ))?;
    registry.register(Box::new(flink::METRIC_CATALOG_FLOW_SOURCE_MILLIS_BEHIND_LATEST.clone()))?;
    registry.register(Box::new(
        flink::METRIC_CATALOG_FLOW_TASK_UTILIZATION_1_MIN_ROLLING_AVG.clone(),
    ))?;
    registry.register(Box::new(
        flink::METRIC_CATALOG_FLOW_SOURCE_TOTAL_LAG_1_MIN_ROLLING_AVG.clone(),
    ))?;
    registry.register(Box::new(
